...
def render(user, **colors):
    ...
    st.markdown('<span class="st">Administração da Plataforma</span>', unsafe_allow_html=True)

    tab_users, tab_metas, tab_unid, tab_senha = st.tabs([
        "👥 Usuários","🎯 Metas por Unidade","🏭 Unidades / Áreas","🔑 Senhas"
    ])
    ...
    with tab_users:
        ...
        perfis_disp = ["operador","cost_control","gestor","admin"]
        perfis_lbl  = {
            "operador":     "Operador (só vê sua unidade)",
            "cost_control": "Cost Control (valida e lança real — todas unidades)",
        ...
        with st.form("form_user", clear_on_submit=True):
            ...
            with c2:
                perfil_u  = st.selectbox("Perfil *", perfis_disp,
                                          format_func=lambda x: perfis_lbl[x])
                unidade_u = st.selectbox("Unidade (deixe Global para admin/gestor/CC)", nomes_u)
            ...
            if st.form_submit_button("➕ Criar Usuário", use_container_width=True):
                if not nome_u or not email_u or not senha_u:
                    ...
                    try:
                        ...
                        st.success(f"✅ Usuário **{nome_u}** criado!"); st.rerun()
    ...
    with tab_metas:
        ...
        ano_sel = st.selectbox("Ano:", anos, index=anos.index(2026) if 2026 in anos else 0)
    ...
    with tab_unid:
        ...
        st.markdown(f"""
        <table class="dt"><thead><tr><th>Nome</th><th>Tipo</th><th>Status</th></tr></thead>
        ...
        with st.form("form_unid", clear_on_submit=True):
            ...
            if st.form_submit_button("➕ Criar", use_container_width=True):
                if nome_nu:
                    ...
                    st.success(f"✅ {nome_nu} criada!"); st.rerun()
