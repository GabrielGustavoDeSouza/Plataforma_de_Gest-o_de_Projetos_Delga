...
<div class="kpi-grid">
  ...
  <div class="kpi-card" style="border-left-color:{GREEN};
       background:linear-gradient(135deg,#F0FBF4 0%,white 60%);">
    <div class="kpi-l">Retorno Real {ano_sel}</div>
    <div class="kpi-v" style="color:{GREEN};">{fmt_mi(kpi['real'])}</div>
  </div>
  ...
  <div class="kpi-card">
    ...
    nomes_proj   = ["Todos"] + [f"#{p['id']} — {p['nome'][:30]}" for p in projetos_uni]
    proj_map     = {f"#{p['id']} — {p['nome'][:30]}": p for p in projetos_uni}

    c1,c2 = st.columns([3,4])
    with c1:
        series_opts = ["Previsto Mensal","Real Mensal","Acum. Previsto","Acum. Real"]
        series_sel  = st.multiselect("Séries do gráfico:", series_opts,
                                      default=series_opts, key="gr_series")
    with c2:
        proj_sel = st.multiselect("Projetos (vazio = todos):",
                                   nomes_proj[1:], default=[], key="gr_projs",
                                   placeholder="Todos os projetos da unidade")

    # Calcular dados do gráfico
    if proj_sel:
        # Modo isolado: soma só dos projetos selecionados
        ...
        titulo_graf = f"{', '.join(proj_sel[:2])}{'...' if len(proj_sel)>2 else ''}"
    else:
        # Modo unidade completa
    ...
    fig = go.Figure()
    cores_serie = {
        "Previsto Mensal": ("#7EB3D8", "bar"),
        "Real Mensal":     ("#52A97C", "bar"),
        "Acum. Previsto":  (NAVY, "line_dot"),
        "Acum. Real":      (GREEN, "line"),
    }
    dados_serie = {
        "Previsto Mensal": prev_m,
        "Real Mensal":     real_m,
        "Acum. Previsto":  acum_p,
        "Acum. Real":      acum_r,
    }
    for s in series_sel:
        cor, tipo = cores_serie[s]
        dados = dados_serie[s]
        if tipo == "bar":
    ...
    for p in pf:
        ...
        real_acum = get_real_acumulado_projeto(p["id"], ano_sel)
...
<div style="border-left:4px solid {border_c};background:white;
  ...
  <div style="display:flex;align-items:center;gap:16px;">
    <div style="flex:1;min-width:180px;">
      <div style="font-size:10px;color:{SILVER};">{p['tipo']} · {p.get('va_ggf','—')}</div>
    ...
    <div style="display:flex;gap:18px;align-items:center;flex-shrink:0;">
      ...
      <div style="text-align:center;">
        <div style="font-size:9px;color:{SILVER};text-transform:uppercase;letter-spacing:.4px;">Real Acum.</div>
        <div style="font-size:12px;font-weight:700;color:{GREEN};">{fmt_brl(real_acum)}</div>
      </div>
      <div style="text-align:center;">
  ...
  {f'<div style="margin-top:6px;font-size:10px;color:#555;background:#F9F9F9;padding:5px 10px;border-radius:6px;">📌 <b>Atribuição:</b> {p["atividade_atual"]}{(" — "+p["onde_parado"]) if p.get("onde_parado") else ""}</div>' if p.get('atividade_atual') else ''}
        ...
        # Editar — alinhado à direita
        if pode_ed:
            col_esp, col_edit = st.columns([10,2])
            with col_edit:
                expandir = st.button(f"✏️ Editar #{p['id']}",
                                      key=f"btn_ed_{p['id']}",
                                      use_container_width=True)
            if expandir:
        ...
        st.markdown("<hr style='margin:4px 0;border-color:#EEF0F3;'>",
